<x-app-layout>
    <h1 class="text-2xl font-bold mb-6">Recurring Income Logs</h1>
    ...
    <div class="bg-white shadow rounded-xl overflow-hidden">
        <table class="w-full text-sm">
            ...
            <tbody>
                @forelse ($logs as $log)
                    <tr class="border-t">
                        <td class="px-4 py-2">{{ $log->recurringIncome->source }}</td>
                        <td class="px-4 py-2">{{ $log->generated_for_date->format('Y-m-d') }}</td>
                        <td class="px-4 py-2">{{ $log->created_at->format('Y-m-d H:i') }}</td>
                        <td class="px-4 py-2">
                            <a href="{{ route('income.edit', $log->income_id) }}"
                               class="text-blue-600 hover:underline">View Income</a>
                        </td>
                    </tr>
                @empty
